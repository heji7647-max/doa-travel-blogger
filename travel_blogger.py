...
# ==========================================
# 🔒 [보안 업그레이드] 비밀번호와 API 키를 Secrets/환경변수에서 안전하게 자동 추출
# ==========================================
# 1) 비밀번호 검증용 데이터 로드 (코드에 직접 적지 않고 시스템에서 비밀리에 가져옴)
if "MY_APP_PASSWORD" in st.secrets:
    MASTER_PASSWORD = st.secrets["MY_APP_PASSWORD"]
elif "MY_APP_PASSWORD" in os.environ:
    MASTER_PASSWORD = os.environ.get("MY_APP_PASSWORD")
else:
    MASTER_PASSWORD = "doa1234"  # 로컬 PC 최초 세팅용 기본값
...
# 2. 사이드바 설정 (비밀번호 및 연동 현황 확인창)
with st.sidebar:
    st.header("🔒 접근 제어 및 인증")
    
    # 🔐 비밀번호 입력창 (이제 깃허브 코드를 봐도 이 정답은 알 수 없습니다!)
    user_password = st.text_input("프로그램 접속 비밀번호를 입력하세요", type="password")
    st.divider()
...
# ==========================================
# 🔐 3. 비밀번호 검증 후 메인 콘텐츠 렌더링 (도아님 원본 로직 100% 보존)
# ==========================================
if user_password == MASTER_PASSWORD:
    st.title("🏆 Doa 여행 블로그 콘텐츠 생성기")
    st.caption("동행인, 무드, 이동수단 등 디테일한 요소를 선택하여 AI 느낌을 완벽히 지운 '내돈내산' 스타일 글을 생성합니다.")
    st.success("🔓 인증이 완료되었습니다. 모바일 작업실을 마음껏 이용하세요!")
    ...
    st.info("왼쪽 사이드바에 접속 비밀번호를 입력하시면 나만의 모바일 작업실이 열립니다.🔑")
else:
    st.error("❌ 비밀번호가 일치하지 않습니다. 올바른 비밀번호를 입력해 주세요.")
